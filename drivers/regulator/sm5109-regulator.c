...
void sm5109_set_current_for_tablet(void)
{
	int ret = -1;
	int val = i2c_smbus_read_byte_data(sm5109->i2c,SM5109_REG_APPS_DISP_DISN);
	if (val >= 0)
		ret = i2c_smbus_write_byte_data(sm5109->i2c, SM5109_REG_APPS_DISP_DISN, val | 0x40);

	if (val < 0 || ret < 0)
		dev_err(sm5109->dev, "set current failed! [0x%02x, %d, %d]\n", SM5109_REG_APPS_DISP_DISN, val, ret);
}
EXPORT_SYMBOL_GPL(sm5109_set_current_for_tablet);
