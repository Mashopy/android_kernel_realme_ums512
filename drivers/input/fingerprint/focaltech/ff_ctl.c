...
static int ff_ctl_enable_irq(bool on)
{
    ...
    //pr_debug("irq: '%s'.", on ? "enable" : "disabled");
...
static const char *ff_ctl_get_version(void)
{
    ...
    static char final_version[FF_DRV_VERSION_LEN] = {'\0', };
    FF_LOGV("'%s' enter.", __func__);
...
#endif
    sprintf(final_version, "%s-%s", version, ff_ctl_arch_str());
    FF_LOGD("version: '%s'.", final_version);
