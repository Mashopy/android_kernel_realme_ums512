...
long egisfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_RESET_SET_HIGH:
		DEBUG_PRINT(" %s : FP_RESET_SET_HIGH \n", __func__);
		retval = do_egisfp_reset_set(egis_dev, 1);
		break;
	case FP_RESET_SET_LOW:
		DEBUG_PRINT(" %s : FP_RESET_SET_LOW \n", __func__);
		retval = do_egisfp_reset_set(egis_dev, 0);
		break;
	case GET_IO_STUS:
		...
		if (!egis_dev->platforminit_done)
		{
			...
			if (egis_dev->ctrl_power)
			{
				if (egis_dev->pwr_by_gpio)
				{
					...
					status = gpio_direction_output(egis_dev->vcc_33v_Pin, 1);
				}
				else
				{
...
int egisfp_parse_dt(struct egisfp_dev_t *egis_dev)
{
	...
	if (node)
	{
		...
		egis_dev->rstPin = of_get_named_gpio(node, "fpreset-gpios",0);
		...
		egis_dev->irqPin = of_get_named_gpio(node, "fpint-gpios", 0);
...
int __init egisfp_init(void)
{
	...
	INFO_PRINT(" %s : module init OK !  status= %d\n", __func__,status);
	return status;
}
