...
void ili_input_register(void)
{
	...
	input_set_capability(ilits->input, EV_KEY, KEY_F4);/*ritchie add for oplus*/

	__set_bit(KEY_F4, ilits->input->keybit);/*ritchie add for oplus*/
...
//add ear phone function
static void ipd_headset_work_queue(struct work_struct *work)
 {
	ILI_INFO("ipd_headset_work_queue +++++headset_state = %ld\n",ilits->headset_state);
	if (ilits->tp_suspend){
		ILI_INFO("suspend = %d,isUpgrading = %d,don't need headset function",ilits->tp_suspend,ilits->fw_upgrade_mode);
		return;
	}
	mutex_lock(&ilits->touch_mutex);
	if(ilits->headset_state == 1) {
		ili_ic_func_ctrl("ear_phone", 0x1);
	} else if(ilits->headset_state == 0) {
		ili_ic_func_ctrl("ear_phone", 0x0);
	} else
		ILI_INFO("invild headset statues value");
	mutex_unlock(&ilits->touch_mutex);
 }

static int ipd_notifie_headset(struct notifier_block *nb,unsigned long value, void *data)
{
	ilits->headset_state = value;
	ILI_INFO("value == %ld,ipd->headset_state = %ld data = %p",value,ilits->headset_state,data);
	schedule_work(&ilits->headset_work_queue);
	return 0;
}
...
{
	int res = 0;
	ILI_INFO("ipd_headset_notifier_init\n");
	ilits->notifier_headset.notifier_call = ipd_notifie_headset;
	res = headset_register_client(&ilits->notifier_headset);
}
...
static int ilitek_plat_probe(void)
{
	...
	INIT_WORK(&ilits->headset_work_queue, ipd_headset_work_queue);
...
static int __init ilitek_plat_dev_init(void)
{
	...
	if (!lcd_str) {
        ILI_INFO("lcd name is null\n");
        return 0;
    }

	if (!(strncmp(lcd_str,"ilt9882n_ls_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_ls_psc_hdp_dsi_vdo_lcm")))) {
		ili_tp_module = MODEL_LS;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(lcd_str,"ilt9882n_truly_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_truly_psc_hdp_dsi_vdo_lcm")))) {
		ili_tp_module = MODEL_TRULY;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(lcd_str,"lcd_ili9882q_skyworth_mipi_hd",strlen("lcd_ili9882q_skyworth_mipi_hd")))) {
		ili_tp_module = MODEL_SKY;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(lcd_str,"lcd_ili7806s_txd_mipi_hd",strlen("lcd_ili7806s_txd_mipi_hd")))) {
		ili_tp_module = MODEL_TXD;
		ILI_INFO("this is ili7806s ilitek touchscreen\n");
	} else {
		ILI_INFO("nothing match lcd name\n");
		return -1;
	}
    ...
    ILI_INFO("==========boot_mode = %d\n", boot_mode);

    if (boot_mode == MSM_BOOT_MODE__CHARGE || boot_mode == MSM_BOOT_MODE__CALI) {
        ILI_INFO("boot_mode is CHARGE and CALI not need to add tp driver\n");
        return 0;
    } else {
        ILI_INFO("boot_mode is normal mode %d\n", boot_mode);
    }
