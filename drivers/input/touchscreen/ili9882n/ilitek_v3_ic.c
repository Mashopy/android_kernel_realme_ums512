...
out:
	...
	if (ili_tp_module == MODEL_LS) {
		sprintf(Ctp_name,"LS,ILI9882,FW:0x%x\n",buf[3]);
		sprintf(ili9882_version,"LS_Ili9882_%0x",buf[3]);
	} else if (ili_tp_module == MODEL_TRULY) {
		sprintf(Ctp_name,"TRULY,ILI9882,FW:0x%x\n",buf[3]);
		sprintf(ili9882_version,"TRULY_Ili9882_%0x",buf[3]);
	} else if (ili_tp_module == MODEL_SKY) {
		sprintf(Ctp_name,"SKYWORTH,ILI9882,FW:0x%x\n",buf[3]);
	} else if (ili_tp_module == MODEL_TXD) {
		sprintf(Ctp_name,"TXD,ILI7806s,FW:0x%x\n",buf[3]);
	} else {
		sprintf(Ctp_name,"DEFAULT,ILI9882,FW:0x%x\n",buf[3]);
		sprintf(ili9882_version,"DEFAULT_Ili9882_%0x",buf[3]);
	}

	ilits->chip->fw_ver = buf[1] << 24 | buf[2] << 16 | buf[3] << 8 | buf[4];
	ilits->chip->fw_mp_ver = buf[5] << 24 | buf[6] << 16 | buf[7] << 8 | buf[8];

	tp_register_devinfo(buf[3]);
	return ret;
}
