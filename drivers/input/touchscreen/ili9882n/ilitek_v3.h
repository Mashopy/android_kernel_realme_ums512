...
#define LS_FW_SIGN_FILP_PATH         "tp/206A1/ls_ili9882n_oplus_signed.bin"
#define TRULY_FW_SIGN_FILP_PATH      "tp/206A1/truly_ili9882n_oplus_signed.bin"
...
/*ritchie add for oplus*/
#define OPLUS_DBG(level, fmt, arg...)						\
	do {									\
		if (level & ilits->oplus_debug_level)						\
		pr_info("OPLUS: (%s, %d): " fmt, __func__, __LINE__, ##arg);	\
	} while (0)
...
/*ritchie add for oplus*/
	...
	struct notifier_block notifier_suspend;
...
struct gesture_coordinate {
	uint8_t gesture_type;/*ritchie add for oplus*/
	u16 code;
	u8 clockwise;
	int type;
...
extern int ili_oplus_proc_init(void);
extern int ili_dev_mkdir(char *name, umode_t mode);
...
/*ritchie add for oplus*/
extern int ili9882n_get_gesture_info(u8 *buf, struct gesture_coordinate * gesture);
/*ritchie add for oplus*/
