...
static void ilitek_resume_by_ddi_work(struct work_struct *work)
{
	...
	mode = ilits->sleep_mode;
	ILI_INFO("Sleep Mode = %d,sense_stop:%d\n", mode, sense_stop);
	switch(mode) {
	case TOUCHSCREEN_SUSPEND:
...
static int ili_pannel_notify_callback(struct notifier_block *nb,
                               unsigned long mode, void *data)
{
	 ...
	 * after TP reset. We then create a wq doing host download before resume.
	 */
	ilits->sleep_mode = mode;
	ILI_INFO("tp sleep mode == %ld start called by ddi\n",mode);
	if (mode == TOUCHSCREEN_RESUME) {
		//atomic_set(&ilits->fw_stat, ENABLE);
		ili_reset_ctrl(ilits->reset);
		ili_ice_mode_ctrl(ENABLE, OFF);
		ilits->ddi_rest_done = true;
...
static void ilitek_rebuild_cmd_work_queue(struct work_struct *work)
{
	int cmd = 0;
	int ret = 0;
	...
	if (ilits->tp_suspend){
		ILI_INFO("already suspend no need send cmd\n");
		goto out;
	}

	if(ilits->game_switch_mode){
		ILI_INFO("enable game switeh\n");
		cmd = 0x00;
		ret = ili_ic_func_ctrl("lock_point", cmd);
		if(ret < 0)
	...
	if (ilits->direction == 0) {
		cmd = 0x01;
		ret = ili_ic_func_ctrl("edge_palm", cmd);
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
	} else if (ilits->direction == 1) {
		cmd = 0x02;
		ret = ili_ic_func_ctrl("edge_palm", cmd);	
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");

	} else if (ilits->direction == 2) {
		cmd = 0x00;
		ret = ili_ic_func_ctrl("edge_palm", cmd);	
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
...
static void ilitek_tddi_wq_init(void)
{
	...
	ilits->notifier_suspend.notifier_call = ili_pannel_notify_callback;
	res = tp_pannel_register_client(&ilits->notifier_suspend);
#endif
	INIT_WORK(&ilits->rebuild_cmd, ilitek_rebuild_cmd_work_queue);
...
int ili_fw_upgrade_handler(void *data)
{
	...
	schedule_work(&ilits->rebuild_cmd);
	if (!ilits->boot) {
		ilits->boot = true;
...
//		ili_report_gesture_mode(trdata, rlen);
		if(ili9882n_get_gesture_info(trdata, ilits->gcoord)){
...
//			input_sync(core_fr->input_device);
			input_report_key(ilits->input, KEY_F4, 1);
			input_sync(ilits->input);
			input_report_key(ilits->input, KEY_F4, 0);
			input_sync(ilits->input);
		}else{
			ILI_ERR("gesture err\n");
		}
...
static void ili_update_tp_module_info(void)
{
	...
	case MODEL_LS:
		...
		ilits->md_sign_fw_rq_path = LS_FW_SIGN_REQUEST_PATH;
	...
	case MODEL_TRULY:
		...
		ilits->md_sign_fw_rq_path = TRULY_FW_SIGN_REQUEST_PATH;
		...
		break;
	case MODEL_SKY:
		ilits->md_name = "SKYWORTH";
		ilits->md_fw_filp_path = SKY_FW_FILP_PATH;
		ilits->md_fw_rq_path = SKY_FW_REQUEST_PATH;
		ilits->md_ini_path = SKY_INI_NAME_PATH;
		ilits->md_ini_rq_path = SKY_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_SKY;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_SKY);
		break;
	default:
		break;
	}
	...
	if (module == 0 || ilits->md_fw_ili_size < ILI_FILE_HEADER) {
		...
		ilits->md_sign_fw_rq_path = DEF_FW_SIGN_REQUEST_PATH;
