...
#include "cts_xl_builtin_firmware.h"
#define NUM_DRIVER_BUILTIN_FIRMWARE_XL ARRAY_SIZE(cts_xl_driver_builtin_firmwares)
...
struct cts_upgrade_fw_info cts_fw_list[] = {
    {"LS", CFG_CTS_FIRMWARE_FILEPATH, cts_driver_builtin_firmwares, NUM_DRIVER_BUILTIN_FIRMWARE},
    {"XL", CFG_CTS_FIRMWARE_FILEPATH_XL, cts_xl_driver_builtin_firmwares, NUM_DRIVER_BUILTIN_FIRMWARE_XL},
};
...
#endif
{
	...
	tp_register_devinfo(cts_data->cts_dev.fwdata.version);
    ...
    if (ret) {
        cts_err("Vendor init failed %d", ret);
...
static int __init cts_driver_init(void)
{
    ...
    if (!lcd_str) {
        cts_info("lcd name is null\n");
        return 0;
    }

    if (!(strncmp(lcd_str,"lcd_icnl9911c_boe_mipi_hd",strlen("lcd_icnl9911c_boe_mipi_hd")))) {
		cts_tpmodule = 0;
        cts_info("This is lcd_icnl9911c_boe_mipi_hd\n");
    } else if (!(strncmp(lcd_str,"lcd_icnl9911c_truly_mipi_hd",strlen("lcd_icnl9911c_truly_mipi_hd")))) {
		cts_tpmodule = 1;
		cts_info("This is lcd_icnl9911c_truly_mipi_hd\n");
    } else {
        cts_err("nothing match lcd name\n");
        return 0;
    }

    boot_mode = mode_name;

    cts_info("==========boot_mode = %d\n", boot_mode);

    if (boot_mode == MSM_BOOT_MODE__CHARGE || boot_mode == MSM_BOOT_MODE__CALI) {
        cts_info("boot_mode is CHARGE and CALI not need to add tp driver\n");
        return ret;
    } else {
        cts_info("boot_mode is normal mode %d\n", boot_mode);
    }
